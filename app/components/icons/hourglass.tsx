...
const Hourglass: React.FC<IconProps> = ({ 
  color = "var(--color-prime-500)",
  size = "28",
}) => {
  const calculatedSize = size; 
  const sizeString = typeof calculatedSize === 'number' ? `${calculatedSize}px` : calculatedSize;
  const svgStyle = {
    width: sizeString,
    height: sizeString,
  };
  ...
  return (
    <svg
      ...
      style={svgStyle}
    >
      <mask id="mask0_424_1447" style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="24">
        <rect width="24" height="24" fill={color} />
      </mask>
      <g mask="url(#mask0_424_1447)">
        <path
